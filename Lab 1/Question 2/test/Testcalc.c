...
void test_add()
{
	...
	int boundary_test = add(100, 100);
	TEST_ASSERT_EQUAL(200, boundary_test);

	int overflow_test = add(100, 50);
	TEST_ASSERT_EQUAL(150, overflow_test);
}
...
void test_false_as_negative()
{
	bool output = negnum(3);
	TEST_ASSERT_EQUAL(false, output);
}

void test_true_as_negative()
{
	bool output = negnum(-3);
...
int main(void)
{
	...
	RUN_TEST(test_false_as_negative);
	RUN_TEST(test_true_as_negative);
